# "calculate_nps_from_mrc_CTF_zeros_v2.5.py"
#!/usr/bin/env python3
"""
...
def plot_nps_profile(raw_radii, raw_nps, final_radii, final_nps, output_path, box_size, pixel_size):
    if not PLOTTING_ENABLED: print("⚠️ Plotting disabled. `matplotlib` and `seaborn` are required."); return
    
    # --- Convert x-axis from pixels to physical units (Angstroms^-1) ---
    def px_to_inv_angstrom(k_px):
        return np.divide(k_px, (box_size * pixel_size), where=k_px!=0)

    raw_radii_A_inv = px_to_inv_angstrom(raw_radii)
    final_radii_A_inv = px_to_inv_angstrom(final_radii)

    plt.style.use('seaborn-v0_8-whitegrid')
    fig, ax = plt.subplots(figsize=(12, 7))
    
    # Exclude k=0 point from plotting for better visualization
    ax.scatter(raw_radii_A_inv[1:], raw_nps[1:], s=15, alpha=0.5, label='Raw Binned Data', color='cornflowerblue')
    ax.plot(final_radii_A_inv[1:], final_nps[1:], 'r-', lw=2.5, label='Final Analytical Model Fit (MLE)')
    
    ax.set_title('1D Noise Power Spectrum Profile (from CTF Zeros)', fontsize=16)
    ax.set_xlabel('Spatial Frequency (Å⁻¹)')
    ax.set_ylabel('Average Noise Power')
    ax.set_xscale('log'); ax.set_yscale('log'); ax.legend()
    ax.grid(True, which="both", ls="--")

    # --- Force numeric labels on all ticks (major and minor) ---
    ax.xaxis.set_major_formatter(ticker.FormatStrFormatter('%.2f'))
    ax.xaxis.set_minor_formatter(ticker.FormatStrFormatter('%.2f'))
    ax.tick_params(axis='x', which='minor', rotation=45)

    # --- Set left x-axis limit, allowing Matplotlib to auto-scale the right (original behavior) ---
    if len(raw_radii) > 1:
        left_limit_px = max(0.5, raw_radii[1] / 2)
        left_limit_A_inv = px_to_inv_angstrom(left_limit_px)
        ax.set_xlim(left=left_limit_A_inv)
...
def main():
    ...
    parser.add_argument('--exclude_low_freq_bins', type=int, default=1,
                        help="Number of radial bins to exclude from the start (low frequencies) of the fit. (Default: 1 to exclude DC).")
    parser.add_argument('--exclude_high_freq_bins', type=int, default=0,
                        help="Number of radial bins to exclude from the end (high frequencies) of the fit. (Default: 0).")
    ...
    # Calculate the average power PER SAMPLE
    ...
    if args.exclude_low_freq_bins < 0 or args.exclude_high_freq_bins < 0:
        sys.exit("❌ Error: --exclude_..._bins must be a non-negative integer.")

    fit_successful = False
    try:
        print("  - Fitting analytical model using Maximum Likelihood Estimation (MLE)...")

        def negative_log_likelihood(params, freqs, nps_data, counts_data):
            """
            Objective function for MLE. Assumes the power at a given frequency
            follows an exponential distribution with mean=NPS(freq).
            The log-likelihood for a bin 'k' is:
            log L_k = -M_k * log(NPS(k)) - S_k / NPS(k)
            where M_k is the number of samples and S_k is the sum of powers.
            We minimize the negative of the sum of log L_k over all bins.
            """
            nps_model = nps_model_advanced(freqs, *params)
            nps_model = np.maximum(nps_model, 1e-9)
            sum_powers_data = nps_data * counts_data
            log_L_terms = counts_data * np.log(nps_model) + sum_powers_data / nps_model
            return np.sum(log_L_terms)

        start_idx = args.exclude_low_freq_bins
        end_idx = len(radii_bins_valid) - args.exclude_high_freq_bins
            
        if end_idx <= start_idx or (end_idx - start_idx) < 10:
             raise RuntimeError(f"Not enough data points to fit after excluding bins. Range: {start_idx}-{end_idx}")

        freqs_fit = radii_bins_valid[start_idx:end_idx]
        nps_profile_fit = nps_profile_1d_valid[start_idx:end_idx]
        counts_fit = counts_accumulator[valid_bins][start_idx:end_idx]
        print(f"  - Fitting on {len(freqs_fit)} data points (bins {start_idx} to {end_idx-1}).")

        initial_C = np.mean(nps_profile_fit[-int(len(nps_profile_fit) * 0.1):]) if len(nps_profile_fit) > 10 else nps_profile_fit[-1]
        initial_A = np.max(nps_profile_fit) - initial_C

        initial_guesses = [
            initial_A, 5.0, 2.0, input_size / 2.0, initial_C
        ]
...
#            (input_size / 4.0, input_size * 2), (0, None)
        ...
        result = minimize(negative_log_likelihood, initial_guesses, 
                          args=(freqs_fit, nps_profile_fit, counts_fit),
        ...
        popt = result.x
        nps_profile_1d_final = nps_model_advanced(final_radii, *popt)
        fit_successful = True
        param_names = ['A', 'k0', 'a', 'kd', 'C']
        
        print("  - ✓ Fit successful. Optimal parameters:")
        any_at_boundary = False
        for name, val, bnd in zip(param_names, popt, bounds):
            low_bnd_str = f"{bnd[0]:.4g}" if bnd[0] is not None else "None"
            high_bnd_str = f"{bnd[1]:.4g}" if bnd[1] is not None else "None"
            bnd_str = f"[{low_bnd_str}, {high_bnd_str}]"
            is_at_boundary = False
            tolerance = 1e-4
            if bnd[0] is not None and abs(val - bnd[0]) < tolerance: is_at_boundary = True
            if bnd[1] is not None and abs(val - bnd[1]) < tolerance: is_at_boundary = True
            if is_at_boundary: any_at_boundary = True
            boundary_marker = "  <-- AT BOUNDARY" if is_at_boundary else ""
            print(f"      - {name:<8s} = {val:>9.4f}  (bounds: {bnd_str:<22}) {boundary_marker}")

        if any_at_boundary:
            print("  - ⚠️  Warning: One or more parameters reached their optimization boundary. The fit may be constrained.")

    except (RuntimeError, ValueError) as e:
        print(f"  - ⚠️  Analytical MLE fit failed: {e}. Falling back to smoothing/interpolation.")

    if not fit_successful:
        window_size = max(3, int(len(radii_bins_valid) * 0.02))
        if window_size % 2 == 0: window_size += 1
        print(f"  - Applying running average smoothing with window size: {window_size} bins")
