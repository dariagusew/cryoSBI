...
class NREInferenceModel(nn.Module):
    def __init__(self, encoder: nn.Module, nre: nn.Module, conf_embedding: torch.Tensor):
        super().__init__()
    ...
    with mrcfile.mmap(mrc_path, mode="r") as mrc:
        ...
        for start in tqdm(range(0, N_images, image_batch_size), desc="Image chunks"):
            ...
            for p_start in range(0, B * K, pair_batch_size):
                ...
                batch_theta_one_hot = model.conf_embedding[batch_theta_indices]
...
def main(args):
    ...
    conf_embedding = ckpt["conf_embedding"]
    ...
    nre = NREHead(x_dim=embedding_dim, n_conformations=conf_embedding.size(1))
    inf_model = NREInferenceModel(encoder, nre, conf_embedding).to(device)
