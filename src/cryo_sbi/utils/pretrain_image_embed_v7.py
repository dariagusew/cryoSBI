...
class NREHead(nn.Module):
    ...
    def forward(self, theta: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ...
        # x:     (B, D)
        theta_f = theta.float().unsqueeze(-1)  # (B, 1)
...
class ImageEmbedPretrainModel(nn.Module):
    def __init__(
        ...
        self.nre       = NREHead(x_dim=embedding_dim)
        ...
        print(f"  NRE head:  theta (raw int) + mu (dim={embedding_dim}) -> log r")


    def forward(self, x: torch.Tensor, theta: torch.Tensor):
        mu, log_var, z = self.encoder.forward_vib(x)
        preds = self.predictor(z)
        log_r = self.nre(theta, mu)   # auxiliary NRE on deterministic mu
...
def _run_vib_epoch(
    ...
    for _ in range(n_batches_per_epoch):
        ...
        for i in range(0, n_full, batch_size):
            ...
            N = targets["indices"].size(0)
            theta_i = targets["indices"].unsqueeze(1).expand(N, N).reshape(N * N)
            mu_j = mu.unsqueeze(0).expand(N, N, -1).reshape(N * N, -1)
            log_r_matrix = model.nre(theta_i, mu_j).view(N, N)
            L_nre = nre_loss_fn(log_r_matrix, targets["indices"])

            loss = loss + beta_NRE * L_nre
