...
Pre-train MLP embedding network to distinguish between different conformations.
This creates discriminative embeddings that can then be used in NLE training.

Usage:
    python pretrain_mlp.py --model_file models.npy --output pretrained_mlp.pt
"""
...
def pretrain_mlp(
    ...
    """
    Pre-train MLP to distinguish between different conformations.
    ...
    mlp = EMBEDDING_NETS["MLP"](embedding_dim).to(device)
...
def main():
    parser = argparse.ArgumentParser(
        description="Pre-train MLP embedding network for conformation discrimination",
