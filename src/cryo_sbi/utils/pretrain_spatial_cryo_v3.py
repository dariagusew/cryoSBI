"""
pretrain_spatial_cryo.py

Unsupervised pre-training of SPATIAL_CRYO encoder for cryo-EM.
Uses reconstruction with optional L2 regularization on embeddings.
...
# ============================================================================
# LIGHTWEIGHT DECODER (MIRRORS SPATIAL_CRYO ENCODER)
# ============================================================================
...
class SpatialCryoPretrainModel(nn.Module):
    """SPATIAL_CRYO encoder with symmetric decoder for pretraining"""
    def __init__(self, embedding_name, embedding_dim, image_size):
        ...
        # Create encoder (SPATIAL_CRYO or SPATIAL_CRYO_FFT_FILTER)
...
def pretrain_spatial_cryo(
    ...
    save_path: str = 'pretrained_spatial_cryo.pt',
    ...
    try:
        model = SpatialCryoPretrainModel(embedding_name, embedding_dim, image_size).to(device)
    ...
    print(f"  Encoder/Decoder ratio: {params['encoder']/params['decoder']:.2f}:1")
        ...
        for epoch in tq:
            ...
            for parameters in islice(prior_loader, n_batches_per_epoch):
                ...
                for batch_images in images.split(batch_size):
                    ...
                    # Encourages smaller magnitudes → more compact embedding space
    ...
    # Save encoder weights only (for NLE training)
