...
Usage:
    python validate_embedding_v6.py \
...
class ResidualUNet(nn.Module):
    """Lightweight residual U-Net — must match the architecture in pretrain_image_embed_v5.py."""
...
def generate_and_encode_synthetic(
    ...
    with torch.no_grad():
        while n_collected < n_images:
            ...
            all_embeddings.append(batch_embs[:n_to_keep])
    ...
    embeddings_arr = torch.cat(all_embeddings, dim=0).numpy()
    ...
    return embeddings_arr, indices_arr, example_arr
