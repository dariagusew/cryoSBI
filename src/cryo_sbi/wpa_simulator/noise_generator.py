...
Inference-only wrapper around the trained GAN generator.
...
class NoiseGenerator:
    """
    Inference wrapper for the trained GAN ice-noise generator.
    ...
    Usage
    -----
    gen     = NoiseGenerator(pt_file="noise_gan.pt", device="cuda")
    ...
    def __init__(
        ...
        if not self.pt_file.exists():
            raise FileNotFoundError(f"GAN checkpoint not found: {self.pt_file}")
        ...
        print(
            f"  [NoiseGenerator] Loaded '{self.pt_file.name}' "
            f"on {self.device}  "
