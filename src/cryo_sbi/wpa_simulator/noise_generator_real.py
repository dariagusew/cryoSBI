"""
noise_generator_real.py

Inference-only wrapper around the trained GAN-REAL generator.
"""
...
class NoiseGeneratorReal:
    """
    Inference wrapper for the trained GAN-REAL ice-noise generator.
    ...
    def __init__(
        ...
        if not self.pt_file.exists():
            raise FileNotFoundError(f"GAN-REAL checkpoint not found: {self.pt_file}")
        ...
        print(
            f"  [NoiseGeneratorReal] Loaded '{self.pt_file.name}' "
            f"on {self.device}  "
