...
# Step 1: Pretrain SPATIAL_CRYO_GAUSS_FFT_FILTER
echo "=== Started pretraining SPATIAL_CRYO_GAUSS_FFT_FILTER ==="

pretrain_image_embed \
    --embedding SPATIAL_CRYO_GAUSS_FFT_FILTER \
    --epochs 100 \
    --batch_size 256 \
    --device cuda:1 \
    --output pretrained_spatial_cryo_gauss_filter.pt \
    --embedding_dim 16 \
    --image_config "${BASE_PATH}simulation_parameters.json" \
    --lr 0.0002 \
    --simulation_batch_size 1024

echo "=== Finished pretraining SPATIAL_CRYO_GAUSS_FFT_FILTER ==="
...
train_nle_model \
    ...
    --n_workers 4 \
    --train_device cuda:1 \
    --pretrained_embedding_path pretrained_spatial_cryo_gauss_filter.pt \
